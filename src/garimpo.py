"""Orquestrador: varre o OLX pelos modelos monitorados e devolve os achados
(motos do ABC, sem leilao, km <= limite, abaixo da FIPE no minimo pedido).
...
def garimpar(cfg, log=print):
    achados = []
    paginas = cfg.get("paginas_por_modelo", 3)
    for modelo in cfg["modelos"]:
        ...
        for a in abc:
            ...
            if not a.get("ano"):
                continue
            # 3) km <= limite (JSON do OLX ja traz km; regra estrita)
            if not filtros.passa_km(a, cfg["km_max"]):
                continue

            # 4) referencia FIPE (usa modelo normalizado do OLX + tokens que travam a familia)
